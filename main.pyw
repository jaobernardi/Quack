...
character = {
    "pos": {"x": 0, "y": 0}, "dimensions": {"length": 50, "height": 50},
...
def character_move():
    ...
    pygame.draw.rect(win, (0, 0, 0), (
        character["pos"]["x"], character["pos"]["y"], character["dimensions"]["height"],
        character["dimensions"]["length"]))

    # Clear Last Position
    gravity = gravity_check()

    # Phisycs check (MIRRRRRRRRRRRRRRRRRRRR)
    # Gravity
    if gravity:
...
def main_loop():
    ...
    myfont2 = pygame.font.SysFont("Consolas", 20)
    while global_status["running"]:
        ...
        if pygame.key.get_pressed()[pygame.K_ESCAPE]:
            ...
            sleep(0.5)
        if not global_status["paused"]:
            print(global_status["paused"])
            character_move()
            for I in plataforms: pygame.draw.rect(win, (255, 0, 0), (I[0], I[1], 1, 1))
            if label2: win.blit(label2, (100, 100))
            tick = character["pos"]
            label = myfont.render(f"{tick}", 1, (255, 0, 0))
            label2 = myfont2.render(f'{tick}', 1, (0, 0, 0))
            win.blit(label2, (100, 100))
